...
import { ErrorCode, MCP_TRANSPORT_PATH, JSON_RPC_VERSION, MCP_REQUEST_TIMEOUT } from './constants/mcp';
...
function main(): void {
  ...
  app.post(MCP_TRANSPORT_PATH, handleMCPRequest);

  app.delete(MCP_TRANSPORT_PATH, async (req: Request, res: Response) => {
    try {
      const sessionId = req.headers[HEADER_MCP_SESSION_ID] as string;
      ...
      console.error(`Error in DELETE ${MCP_TRANSPORT_PATH}:`, error);
      res.status(500).send('Internal server error');
    }
  });

  app.all(MCP_TRANSPORT_PATH, async (req: Request, res: Response) => {
  ...
  app.listen(port, () => {
    ...
    console.log(`Modern endpoint: http://localhost:${port}${MCP_TRANSPORT_PATH}`);
  });
}
