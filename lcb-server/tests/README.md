...
│       ├── cache.js               # Mock for fastly:cache (with TTL support)
...
│   └── mcp.test.ts                # MCP constants tests (includes SESSION_TTL_SECONDS)
...
├── session-manager.test.ts        # SessionManager lifecycle and TTL tests
...
The `session-manager.test.ts` file contains comprehensive tests for the MCP session lifecycle:

### Test Coverage

- **Constructor**: Default and custom TTL initialization
- **Session Creation**: `setSession()` with single and multiple sessions
- **Session Retrieval**: `getSession()` with existence and expiration checks
- **Session Deletion**: `deleteSession()` with error handling
- **TTL Configuration**: Verification of 1800-second (30-minute) default
- **Session Expiration**: Automatic cleanup after TTL elapsed
- **Multiple Operations**: Concurrent session handling
- **Edge Cases**: Long IDs, special characters, case sensitivity
...
The `SESSION_TTL_SECONDS` constant (defined in `server/src/constants/mcp.ts`) controls the session lifetime:
