...
export const widgetMeta = {
  uri: "ui://eds-widget/my-widget.html",
  name: "My Widget",
  description: "Description of what this widget displays",
  mimeType: "text/html+skybridge",
  htmlFile: "myWidget.html",
  _meta: {
    "openai/outputTemplate": "ui://eds-widget/my-widget.html",
...
- **`uri`**: Unique identifier following the pattern `ui://eds-widget/widget-name.html`
...
EDS Widgets should follow consistent URI patterns:

- **EDS Widgets**: `ui://eds-widget/widget-name.html`
...
### 3. Build Process
The build script (`generate-eds-widgets-index.js`) automatically:
...
- **EDS Widget**: `myWidgetTool.ts` (same name)
- **URI**: `ui://eds-widget/my-widget-action.html`
...
### Manual Generation
```bash
# Generate EDS Widgets index only
make generate-eds-widgets

# Generate both actions and EDS Widgets
make generate-all
```
...
3. **Index generated**: `make generate-eds-widgets` has been run
...
3. Connect to: `http://localhost:${LCB_SERVER_PORT}/<service-name>` (from `.env`)
...
**Note**: The MCP endpoint path is extracted at build time from `AEM_COMPUTE_SERVICE` in `.env` using `getMCP_TRANSPORT_PATH()` in `../constants/mcp.ts`. Change `.env` and rebuild to update the path.
